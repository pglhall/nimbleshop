...
So Steve (buyer) click on 'checkout with PayPal' because he wants to use paypal to pay for the order. The shop takes Steve to PayPal site. Here Steve makes the payment. After Steve makes the payment, Steve is taken back to the shop and everything looks good.

Steve made a successful payment at PayPal. PayPal tells Steve all is good. So Steve is happy.

However now the question is how would shop know that Steve indeed paid the full amount at PayPal. Until the shop is sure that Steve paid in full at PayPal items cannot be shipped to him.
...
Internet is not the most reliable medium when it comes to processing requests. HTTP requests get lost all the time. To ensure that the shop gets the IPN, PayPal keeps sending the IPN for 4 days. Inorder to stop PayPal sending the IPN the shop needs to tell PayPal that it has received the IPN. This is how it works.

* PayPal sends IPN to the shop.
* Shop sends the same data in the same order back to PayPal.
* PayPal sends either 'VERIFIED' or 'INVALID'. If PayPal sends 'VERIFIED' then it means it is all good and now PayPal will not send IPN for that transaction.

When PayPal sends IPN multiple times all those IPNs carry the same txn_id. That txn_id is stored to check if the IPN is a duplicate or not .Duplicate IPN should be discarded.
