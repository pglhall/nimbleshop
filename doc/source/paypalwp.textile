...
* Start rails server at port 3000 by executing +rails server+ .
* Open another terminal. Go to the nimbleShop project directory and execute +bundle exec localtunnel -k ~/.ssh/id_rsa.pub 3000+ . Sometimes you might get message +[Error] Unable to register tunnel. Perhaps service is down?+. It means localtunnel is not working and you will not be able to test Instant Payment Notification ( IPN ). However you can still test making payment part .
